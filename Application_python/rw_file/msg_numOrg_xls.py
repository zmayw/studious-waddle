...
for index in range(rowsCount):
    msgNum = table.row_values(index)[0]
    orgId = table.row_values(index)[1]
    print msgNum
    print orgId
    ...
    if len(results_msgNum)==0:
       ...
       sql_card = "select id,title,card_content,audit_status from msg_card where msg_number_id='%s' " \
                  "and audit_status in ('online','offline') and (channel_id='1006585400001' or channel_id='dfee55ef6e5d4836959c01519f6624e1') and `remove`=0;" % msgNumId
      ...
      # print str(results_card).decode('unicode')
       if len(results_media)!=0:
            ws.write(index,3,"http://file.cctocloud.com/"+str(results_media[0][0]).decode("unicode_escape"))
       ws.write(index,4,str(results_menu).decode("unicode_escape"))
       ws.write(index,5,str(results_card).decode("unicode_escape"))
wb.save("query_result.xls")
cursor.close()
conn.close()